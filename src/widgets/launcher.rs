...
use super::{
    BAR_POPUP_WIDTH, CssTransition, PopupReveal, SmoothScrollConfig, attach_popup_escape_handler,
    attach_popup_lifecycle, build_bar_popup, clear_box, detach_application_window,
    install_smooth_scroll, reset_hidden_popup_state, run_when_popup_visible,
};
...
const FAVORITES_REVEAL_DURATION_MS: u32 = 220;
...
impl LauncherCatalog {
    ...
    fn refresh_if_stale(self: &Rc<Self>) {
        ...
        if !self.initialized.get() || current_visible_app_keys() != self.visible_app_keys() {
    ...
    fn visible_app_keys(&self) -> HashSet<String> {
        self.data
            .borrow()
            .apps
            .iter()
    ...
    fn notify_user_state_changed(&self) {
        for controller in self.live_controllers() {
            controller.user_state_changed();
        }
    }
...
struct LauncherController {
    ...
    category_buttons: Vec<(LauncherCategory, gtk::ToggleButton)>,
    favorites_revealer: gtk::Revealer,
...
impl LauncherIndicator {
    pub fn new(
        ...
        let favorites_slot = gtk::Box::new(gtk::Orientation::Horizontal, 0);
        favorites_slot.add_css_class("launcher-category-favorite-slot");
        favorites_slot.set_overflow(gtk::Overflow::Hidden);

        let favorites_revealer = gtk::Revealer::new();
        favorites_revealer.set_transition_type(gtk::RevealerTransitionType::SlideRight);
        favorites_revealer.set_transition_duration(FAVORITES_REVEAL_DURATION_MS);
        favorites_revealer.set_reveal_child(false);
        favorites_revealer.set_child(Some(&favorites_slot));
        category_bar.append(&favorites_revealer);
        ...
        let category_buttons = LauncherCategory::BUTTONS
            ...
            .map(|category| {
                ...
                button.set_active(category == LauncherCategory::All);
                if category == LauncherCategory::Favorites {
                    button.add_css_class("launcher-category-favorite-button");
                    button.set_hexpand(false);
                    button.set_sensitive(false);
                    button.set_focusable(false);
                    favorites_slot.append(&button);
                } else {
...
impl LauncherController {
    fn connect(this: &Rc<Self>, bar_window: &gtk::ApplicationWindow) {
        ...
        for (category, button) in this.category_buttons.iter().cloned() {
            ...
            button.connect_clicked(move |_| {
                ...
                if previous == category && !searching {
                    return;
                }
    ...
    fn user_state_changed(self: &Rc<Self>) {
        self.reload_user_state();
        self.ensure_valid_category();
        if self.popup.is_visible() {
            self.sync_category_buttons();
    ...
    fn toggle_state_key(&self, kind: LauncherStateKind, key: &str) {
        let mut keys = read_state_keys(kind.file_name());
        if !keys.remove(key) {
            keys.insert(key.to_owned());
        }
        save_state_keys(kind.file_name(), &keys);

        let mut state = self.user_state.borrow_mut();
        *kind.keys_mut(&mut state) = keys;
        self.row_state_dirty.set(true);
        drop(state);
        self.catalog.notify_user_state_changed();
    }
    ...
    fn sync_category_buttons(&self) {
        ...
        // GtkRevealer animates the favorite slot's requested width. As a
        // result, the category row and popup grow/shrink smoothly instead of
        // jumping when the first favorite is added or the last one is removed.
        self.favorites_revealer.set_reveal_child(has_favorites);
    ...
    fn build_app_row(self: &Rc<Self>, app: Rc<LaunchableApp>) -> LauncherAppRow {
        ...
        hidden_button.set_child(Some(&hidden_icon));
...
fn save_state_keys(file_name: &str, keys: &HashSet<String>) {
