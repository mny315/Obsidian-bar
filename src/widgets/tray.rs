...
impl TrayItem {
    ...
    fn open_context_menu(self: &Rc<Self>, anchor: &gtk::MenuButton) {
        let menu_path = cached_property::<glib::variant::ObjectPath>(&self.proxy, "Menu")
            .map(|path| path.as_str().to_owned())
            .filter(|path| path != "/");
        let Some(menu_path) = menu_path else {
            call_item_method(&self.proxy, "ContextMenu", Some(&(0i32, 0i32).to_variant()));
            return;
        };
        ...
        if let Some(proxy) = cached {
            self.load_context_menu(anchor, proxy);
            return;
        }
            ...
            move |result| {
                ...
                match result {
                    Ok(proxy) => {
                        ...
                        this.load_context_menu(&anchor, proxy);
                    }
                    Err(error) => {
    ...
    fn load_context_menu(self: &Rc<Self>, anchor: &gtk::MenuButton, menu_proxy: gio::DBusProxy) {
            ...
            move |_| {
                ...
                menu_proxy_for_layout.call(
                    ...
                    move |result| {
                        ...
                        {
                            show_menu_popover(&anchor, &menu_proxy_for_reply, &root.children);
                        } else {
                            call_item_method(
                                &fallback_proxy_for_reply,
                                "ContextMenu",
                                Some(&(0i32, 0i32).to_variant()),
                            );
                        }
...
fn show_menu_popover(anchor: &gtk::MenuButton, proxy: &gio::DBusProxy, nodes: &[MenuNode]) {
...
fn build_menu_box(
    ...
    for node in nodes.iter().filter(|node| node.visible) {
        ...
        if !node.children.is_empty() {
            ...
            submenu.set_child(Some(&build_menu_box(proxy, &node.children, root_popover)));
